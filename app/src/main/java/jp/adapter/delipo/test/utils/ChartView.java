package jp.adapter.delipo.test.utils;
...
import static jp.adapter.delipo.test.utils.Utils.createPaint;
import static jp.adapter.delipo.test.utils.Utils.createPoint;
import static jp.adapter.delipo.test.utils.Utils.createPoints;
import static jp.adapter.delipo.test.utils.Utils.dp;
import static jp.adapter.delipo.test.utils.Utils.gradient;
import static jp.adapter.delipo.test.utils.Utils.mutatePaint;
import static jp.adapter.delipo.test.utils.Utils.sp;
...
public class ChartView extends View {
    ...
    public ChartView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray values = context.obtainStyledAttributes(attrs, R.styleable.ChartView, defStyleAttr, 0);
        startColor = values.getColor(R.styleable.ChartView_startColor, colorPrimaryDark);
        endColor = values.getColor(R.styleable.ChartView_endColor, colorPrimary);
        axisColor = values.getColor(R.styleable.ChartView_axisColor, BLACK);
        axisMax = values.getFloat(R.styleable.ChartView_axisMax, 20);
        axisTick = values.getFloat(R.styleable.ChartView_axisTick, axisMax / 5);
        final DisplayMetrics metrics = getResources().getDisplayMetrics();
        final int textSize = values.getDimensionPixelSize(R.styleable.ChartView_textSize, sp(15, metrics));
        circlesOnly = values.getBoolean(R.styleable.ChartView_circlesOnly, false);
        autoSize = values.getBoolean(R.styleable.ChartView_autoSize, true);
        axisWidth = values.getDimensionPixelSize(R.styleable.ChartView_axisWidth, dp(1, metrics));
        chartColor = values.getColor(R.styleable.ChartView_chartColor, colorAccent);
        chartWidth = values.getDimensionPixelSize(R.styleable.ChartView_chartWidth, dp(3, metrics));
        chartStyle = Paint.Style.values()[values.getInt(R.styleable.ChartView_chartStyle, STROKE.ordinal())];
        smoothGradient = values.getBoolean(R.styleable.ChartView_smoothGradient, false);
        values.recycle();
    ...
    private void buildRings() {
        ...
        rings = new ChartView.Ring[ringsCount];
        if (ringsCount == 1) {
            rings[0] = new ChartView.Ring(axisMaxInternal, axisMaxInternal, startColor);
        } else {
            for (int i = 0; i < ringsCount; i++) {
                rings[i] = new ChartView.Ring(axisTickInternal * (i + 1), axisTickInternal, gradient(startColor, endColor, i, ringsCount));
            }
            rings[ringsCount - 1] = new ChartView.Ring(axisMaxInternal, axisMaxInternal - rings[ringsCount - 2].radius, endColor);
        }
    ...
    private void drawCircles(Canvas canvas) {
        for (final ChartView.Ring ring : rings) {
